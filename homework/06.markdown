...
# Anonymous Course Reviews (Sessions and Storing Data) - __Wednesday, April 1st, by 11PM__
...
* once you're connected with a commandline client (mongo), start inserting documents into a database called <code>hw06</code> and a collection called <code>reviews</code>:
...
And, finally, add the code that connects to the database. We'll connect to the local instance of MongoDB, and we'll use a database called <code>hw06</code> (this will be created for you once you start inserting documents... which you should have done already above!). 
...
* add __another__ user by [using this guide](https://docs.mongodb.com/manual/tutorial/enable-authentication/#create-additional-users-as-needed-for-your-deployment)
	* ⚠️ __make sure to modify the value for db so that it reads as "hw06"__
...
* try logging in again, this time adding user and database (and an option to prompt for a password)
	* `mongo --port 27017  -u "yourNewUserName" hw06 -p`
...
3. `mongodb://username:password@localhost/hw06`
...
4. create a config file and tell  your application to use it by specifying an environment variable when you start your app
	...
	* create a `config.json` that contains a connection string  `mongodb://username:password@localhost/hw06` in the property, `dbconf` 
		* so `config.json ` should look something like {"dbconf":"mongodb://username:password@localhost/hw06"}
